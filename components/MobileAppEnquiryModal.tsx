...
import { X, Check, Loader2, Rocket, Layers, HeartHandshake } from 'lucide-react';

interface MobileAppEnquiryModalProps {
  isOpen: boolean;
  onClose: () => void;
  packageType: {
    title: string;
...
const MobileAppEnquiryModal: React.FC<MobileAppEnquiryModalProps> = ({ isOpen, onClose, packageType }) => {
  ...
  const getIcon = () => {
    if (!packageType) return Rocket;
    if (packageType.title === 'MVP') return Rocket;
    if (packageType.title === 'Full-Scale Custom') return Layers;
    if (packageType.title === 'Growth & Support') return HeartHandshake;
    return Rocket;
  };
  const Icon = getIcon();
    ...
    try {
      const response = await fetch('/api/contact-form', {
        ...
        body: JSON.stringify({
          ...
          helpWith: 'Mobile App Development',
          message: `Mobile App Enquiry: ${packageType?.title || 'General'}

Package: ${packageType?.title || 'Not specified'}
Price: ${packageType?.price || 'Not specified'}
Description: ${packageType?.description || 'Not specified'}

Project Details:
${projectDetails || 'Not provided'}

Submitted from: Mobile App Development service page`,
          source: 'mobile-app-enquiry'
        })
      });
        ...
        <div className="relative w-full max-w-md bg-brand-black border border-brand-border/50 rounded-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
          ...
          <div className="p-8 text-center">
            ...
            <p className="text-brand-muted mb-6">
              We'll be in touch within 24 hours to discuss your {packageType?.title?.toLowerCase() || 'mobile app'} project.
            </p>
        ...
        <div className="p-6 sm:p-8">
          ...
          <div className="flex items-start gap-4 mb-6 pb-6 border-b border-brand-border/30">
            ...
            <div>
              <h2 className="text-xl sm:text-2xl font-display font-bold text-white mb-1">
                {packageType?.title || 'Mobile App Enquiry'}
              </h2>
              <p className="text-brand-muted text-sm">
                {packageType?.price || 'Custom pricing'}
              </p>
            </div>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <textarea
                placeholder="What problem does your app solve? Who is it for? Any key features in mind?"
...
export default MobileAppEnquiryModal;
